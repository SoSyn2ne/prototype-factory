# QA checklist

## Happy path
- One-liner and target action understood within three seconds.
- Core flow works: Paste a deal URL, match exact options, stack coupon and shipping conditions, compare effective price, and publish a proof card.
- Output artifact can be saved or shared.

## Edge cases
- Empty, incomplete, conflicting, and blocked inputs have clear recovery.
- Long Korean/English text and small mobile widths do not overflow.
- Mock data is realistic and clearly non-production.

## Acceptance verification
- All required sections present: deal URL intake; exact-option matcher; coupon stack; shipping and membership costs; price-history strip; shareable verdict card.
- Exactly one design profile used: financial-analyst.
- Theme and source references match meta.json and ideas.md.
- No custom demo exists in Phase A.

